...
Monitor status and health of all nodes of a Galera cluster
    ...
    USAGE: galera-status [--help] [--follow] [--interval=<seconds>] [--hosts=<node1>,<node2>,...] [MySQL options]

    OPTIONS: --help        this help
             --follow      continuously updates information retrieved from monitored nodes
             --interval    refresh interval in seconds for follow-mode (default: 1)
...
database account used has the privilege to change global variables. When changing a node's
weight, the script asks for the node's index and new weight. The first node has the index
**1**, the second node has the index **2** and so on. The node's weight must be specified
as integer value >= 0.
